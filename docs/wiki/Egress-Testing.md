...
| :--- | :--- | :--- |
| `9rtk-proxy-a` | `172.31.0.11` | an HTTP proxy |
| `9rtk-proxy-b` | `172.31.0.12` | a second one, so "went through *a* proxy" and "went through *this* proxy" can be told apart |
| `9rtk-echo` | `172.31.0.20` | the referee: answers with the source address it saw |
...
docker network connect 9rtk-egress_egress <gateway-container>
...
- the pool binding works: `docker logs 9rtk-proxy-a` showed
...
vincule a uma conexão e acompanhe `docker logs -f 9rtk-proxy-a`.
