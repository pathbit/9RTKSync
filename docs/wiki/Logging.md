...
Implemented in [`src/nine_rtksync/logs.py`](https://github.com/pathbit/9RTKSync/blob/master/src/nine_rtksync/logs.py),
...
| :--- | :--- | :--- |
| `LOG_DIR` | `<DB_PATH dir>/logs` | Destination directory. Falls back to `~/.9rtksync/logs` when the database directory is not writable. |
...
- One file, `9rtksync.log`, rotated at **UTC midnight**.
- Rotated files are named `9rtksync.log.YYYY-MM-DD`.
...
**Never touched:** the active `9rtksync.log`, and any file that does not belong to this service.
