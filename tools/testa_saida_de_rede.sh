...
if docker ps --format '{{.Names}}' | grep -qx 9rtk-proxy-b; then
  docker stop 9rtk-proxy-b >/dev/null 2>&1
  sleep 2
  ...
  docker start 9rtk-proxy-b >/dev/null 2>&1
  sleep 3
else
  falha "proxy fora do ar -> a requisicao FALHA" "9rtk-proxy-b nao esta na bancada"
fi
