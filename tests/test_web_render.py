...
class TestDashboardOverHttp(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with sqlite3.connect(cls.db_path) as conn:
            ...
            conn.execute("CREATE TABLE combos (id TEXT PRIMARY KEY, name TEXT, models TEXT, kind TEXT)")
            conn.execute(
                "INSERT INTO providerConnections VALUES (?,?,?,?,?,?)",
                ("c1", "antigravity", "Google Antigravity Pro",
    ...
    def _get(self, path: str):
        req = urllib.request.Request(f"http://127.0.0.1:19293{path}")
    ...
    def test_unauthenticated_access_is_rejected(self):
        with self.assertRaises(urllib.error.HTTPError) as ctx:
            urllib.request.urlopen("http://127.0.0.1:19293/", timeout=5)
...
class TestRemainingValidity(unittest.TestCase):
    ...
    def build(self, payload):
        return ConnectionRecord(
            id="c1", provider="antigravity", name="Antigravity",
            created_at="", updated_at="", data_raw=json.dumps(payload),
        )
