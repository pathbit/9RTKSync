...
class TestApiKeyProviderIntegration(unittest.TestCase):
    ...
    def test_rejected_key_is_never_stamped_as_ok(self):
        """O bug original: testStatus = 'ok' era carimbado sem perguntar a ninguem."""
        provider = ApiKeyProvider(validate_credentials=True, opener=opener_returning(401))
        conn = self.build("groq", {"apiKey": "revogada", "testStatus": "ok"})
    ...
    def test_unreachable_provider_does_not_mark_the_key_invalid(self):
        ...
        conn = self.build("groq", {"apiKey": "boa", "testStatus": "ok"})
        _, data, _ = provider.check_and_refresh(conn)
