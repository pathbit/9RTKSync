...
class TestHealthzResilience(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.server = web_server.start_web_server(
            "127.0.0.1", 19291, cls.db_path, router_url="", settings=cls.settings
        )
        time.sleep(0.3)
    ...
    def test_healthz_responds_ok(self):
        with urllib.request.urlopen("http://127.0.0.1:19291/healthz", timeout=5) as resp:
    ...
    def test_healthz_sends_exactly_one_well_formed_response(self):
        ...
        with urllib.request.urlopen(f"http://127.0.0.1:{19291}/healthz", timeout=5) as resp:
            body = resp.read()
            self.assertEqual(resp.status, 200)
    ...
    def test_client_disconnect_does_not_crash_the_server(self):
        ...
        for _ in range(5):
            s = socket.create_connection(("127.0.0.1", 19291), timeout=5)
    ...
    def test_concurrent_requests_are_served_in_parallel(self):
        ...
        def hit():
            try:
                with urllib.request.urlopen("http://127.0.0.1:19291/healthz", timeout=5) as r:
