    ...
    def setUpClass(cls):
        ...
        with sqlite3.connect(cls.db_path) as conn:
            conn.execute(
                ...
                "id TEXT PRIMARY KEY, provider TEXT, name TEXT, "
                "createdAt TEXT, updatedAt TEXT, data TEXT)"
            )
        ...
        cls.server = web_server.start_web_server(
            "127.0.0.1", PORT, cls.db_path, router_url="", settings=cls.settings
        )
        time.sleep(0.3)
...
class TestSecurityHeadersOnEveryResponse(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.server = web_server.start_web_server(
            "127.0.0.1", cls.PORTA, cls.db_path, router_url="", settings=cls.settings
        )
        time.sleep(0.3)
