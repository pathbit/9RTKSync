...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def serve_healthz(self) -> None:
        ...
        if db_ok and gateway_ok:
            ...
            status, corpo = HTTPStatus.SERVICE_UNAVAILABLE, b"ROUTER_SERVICE_UNREACHABLE"
    ...
    def nome_do_provedor_sso(self) -> str:
        ...
        if not config or not sso.descobre(str(config["issuer"])):
            return ""
    ...
    def pagina_de_pouso(self, lang: str) -> bytes:
        ...
        return render_landing_page(lang)
    ...
    def inicia_oidc(self) -> None:
        ...
        documento = sso.descobre(str(cfg_obj.issuer))
        if not documento:
            # Descoberta quebrada não trava o login local: a tela volta com o
            # formulário de sempre.
            self.anota_falha_de_sso("descoberta do emissor indisponível")
            self.recusa_sso()
            return
    ...
    def recebe_oidc(self) -> None:
        ...
        email, motivo = sso.conclui_login(config=config if isinstance(config, dict) else vars(cfg_obj), estado=estado, parametros=parametros)
        if not email:
            self.anota_falha_de_sso(motivo)
    ...
    def sonda_o_gateway(self, alvo: str) -> bool:
        ...
        return bool(sondar(alvo)["online"])

    def contagem_do_banco(self) -> Dict[str, int]:
        """Quantas conexões e combos o banco do gateway tem agora."""
        return (carregar_painel(self.settings) if self.settings else {}).get("counters", {})

    # -- renderização -------------------------------------------------------

    def collect_dashboard_state(self) -> Dict[str, Any]:
        """Lê tudo o que a página precisa. Roda no servidor: o SQLite nunca sai daqui."""
        # UMA chamada ao `gateway.py`, com as chaves de sempre: daqui para baixo
        # o painel não sabe se o gateway guarda isso em SQLite ou responde por HTTP.
        ...
        return {
            ...
            "modelsState": painel["model_states"]["catalog"],
            "cron": self.cron_scheduler.get_status() if self.cron_scheduler else {"active": False},
            "gateway": {
                ...
                "dbOk": painel["model_states"]["db"] == "ok",
    ...
    def serve_dashboard(self, query: Dict[str, List[str]]) -> None:
        ...
        conteudo = render_dashboard(
            ...
            models_state=estado["modelsState"],
            cron=estado["cron"],
    ...
    def serve_api_status(self) -> None:
        ...
        A MESMA costura da tela, e não uma segunda leitura do gateway -- com
        duas leituras as duas superfícies divergiriam sem ninguém notar.
        """
        painel = carregar_painel(self.settings) if self.settings else {
            "connections": [], "combos": []
        }
