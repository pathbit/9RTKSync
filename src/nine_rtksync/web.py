...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def recebe_oidc(self) -> None:
        ...
        email, motivo = sso.conclui_login(config=config, estado=estado, parametros=parametros)
        if not email:
    ...
    def handle_sso_settings(self, campos: Dict[str, List[str]]) -> None:
        ...
        if novo["enabled"] == "oidc":
            problemas = sso.problemas_da_configuracao(novo, bool(sso.ler_segredo(base_dir)))
            if problemas:
                self.redirect_to_dashboard(
